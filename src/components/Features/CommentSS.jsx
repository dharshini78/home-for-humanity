...
const CommentTS = () => {
  ...
  const fetchComments = async () => {
    try {
      const response = await axios.get(
        "https://api.homeforhumanity.xrvizion.com/shelter/comments?shelterId=4"
      );
  ...
  const fetchTranslations = async () => {
    try {
      const response = await axios.post(
        ...
        {
          shelterName: "SuperadobeShelter",
          langCode: selectedLanguage,
          fileName: "superadobeshelter_comments.json",
        },
        {
          headers: {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      formData.append("shelterId", "4"); // Timber Shelter ID
