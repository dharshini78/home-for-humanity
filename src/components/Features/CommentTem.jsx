...
const CommentTS = () => {
  ...
  const fetchComments = async () => {
    try {
      const response = await axios.get(
        "https://api.homeforhumanity.xrvizion.com/shelter/comments?shelterId=2"
      );
  ...
  const fetchTranslations = async () => {
    try {
      const response = await axios.post(
        ...
        {
          shelterName: "TemporaryShelter",
          langCode: selectedLanguage,
          fileName: "temporaryshelter_comments.json",
        },
        {
          headers: {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      formData.append("shelterId", "2"); // Timber Shelter ID
